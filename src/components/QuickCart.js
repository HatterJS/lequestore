...
function QuickCart() {
  ...
  //get cart items and  status isOpen from redux
  const { cart, isOpenCart } = useSelector((state) => state.cart);
  ...
  return (
    <div className="quickCart" style={!isOpenCart ? { display: 'none' } : { display: 'block' }}>
      <div className="quickCart__shadow">
        <div className="quickCart__shadowArea" onClick={() => dispatch(openCart())}></div>
        <div className="quickCart__content">
          <div className="quickCart__header">
            <h3>АКТИВНІ ЗАМОВЛЕННЯ</h3>
            <div className="addedItem__deleteItemBtn" onClick={() => dispatch(openCart())}>
              {deleteFromCartSvg}
            </div>
          </div>
          <div className="quickCart__itemsBlock">
            {cart.map((obj, index) => (
              <AddedItem key={index} {...obj} />
            ))}
            <div
              className="quickCart__emptyCart emptyCart"
              style={{ display: cart.length ? 'none' : 'flex' }}>
              <Empty onClose={() => dispatch(openCart())}></Empty>
            </div>
          </div>
