...
const sizeSVG = (
  <svg width="30" height="30" viewBox="0 0 37 37" fill="none">
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M2.82842 25.4558L0 28.2843L8.48528 36.7696L36.7696 8.48528L28.2843 4.76837e-07L26.163 2.12132L31.8198 7.77817L31.1127 8.48528L25.4558 2.82843L23.3345 4.94975L26.1629 7.77818L25.4558 8.48529L22.6274 5.65686L20.5061 7.77817L26.163 13.435L25.4558 14.1421L19.799 8.48528L17.6777 10.6066L20.5061 13.435L19.799 14.1421L16.9706 11.3137L14.8492 13.435L20.5061 19.0919L19.799 19.799L14.1421 14.1421L12.0208 16.2635L14.8492 19.0919L14.1421 19.799L11.3137 16.9706L9.19239 19.0919L14.8492 24.7487L14.1421 25.4558L8.48528 19.799L6.36396 21.9203L9.19238 24.7487L8.48528 25.4558L5.65685 22.6274L3.53553 24.7487L9.19238 30.4056L8.48528 31.1127L2.82842 25.4558Z"
      fill="#FFC34F"
    />
  </svg>
);
...
function GoodsCard() {
  ...
  React.useEffect(() => {
    ...
    async function getData() {
      try {
        await axios
          .get(`http://185.237.204.125:9999/goods/${itemId}`)
          .then((res) => (res.data ? setGoodsItem(res.data) : history('/')));
      } catch (error) {
  ...
  function buttonTitle() {
    //заголовок кнопки в зависимости от обстоятельств
    if (timeCounter > 0) {
      //таймер задержки после нажатия
      return timeCounter + ' сек.';
    } else if (Boolean(goodsItem.size.length) & !checkedSize) {
      //не выбран размер товара
      return 'Оберіть розмір';
    } else {
      //товар готов к добавлению в корзину
        ...
        <div className="goodsCard">
          <div className="goodsCard__content">
            <div className="goodsCard__image">
              <Carousel>
                {goodsItem.goodsImage.map((obj) => (
                  <Carousel.Item key={obj}>
                    <img className="h-100" src={obj} alt="First slide" />
                  </Carousel.Item>
                ))}
              </Carousel>
            </div>
            <div className="goodsCard__order">
              ...
              <div className="goodsCard__count">
                ...
                <div className="goodsCard__counter counter">
                  <div
                    ...
                    onClick={() => goodsAmount > 1 && setGoodsAmount(goodsAmount - 1)}>
                    -
                  </div>
                  {goodsAmount}
                  <div className="unselectable" onClick={() => setGoodsAmount(goodsAmount + 1)}>
                    +
                  </div>
                </div>
              </div>
              <div className="goodsCard__accept">
                <button
                  ...
                  disabled={
                    Boolean(goodsItem.size.length) & !checkedSize || timeCounter > 0 ? true : false
                  }
                  onClick={addedToCart}>
                  {buttonTitle()}
                </button>
                <button
                  className="acceptButton"
                  disabled={Boolean(goodsItem.size.length) & !checkedSize ? true : false}
                  onClick={() => setWarning(true)}>
                  {Boolean(goodsItem.size.length) & !checkedSize
                    ? 'Оберіть розмір'
                    : 'Купити в один клік'}
                </button>
              </div>
          ...
          {warning && (
            <div className="goodsCard__warning">
              <div className="goodsCard__warningNotice unselectable">
                ...
                <div>
                  <Link to="/order">
                    <button className="acceptButton" onClick={oneClick}>
                      Продовжити
                    </button>
                  </Link>
                  <button className="cancelButton" onClick={() => setWarning(false)}>
                    Відмінити
                  </button>
                </div>
              </div>
